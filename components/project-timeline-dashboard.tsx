  ...
  const fetchReleasesForProject = async (projectSlug: string) => {
    ...
    try {
      const res = await fetch(
        `/api/releases/recent?projectSlug=${encodeURIComponent(projectSlug)}`,
      );
      const data = await res.json();
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative flex items-center">
            ...
            >
              ...
              <option value="IMPORT_NEW">+ Import New Project...</option>
            </select>
            <CaretDownIcon
              className="h-3.5 w-3.5 absolute right-2.5 pointer-events-none text-muted-foreground"
              weight="bold"
            />
          </div>
        </div>
          ...
          <div className="flex items-center justify-between border-b border-border pb-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Multi-audience changelog feed for{" "}
                {selectedProject?.githubRepo || selectedProject?.name}
              </p>
            </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    New commits pushed to default branch since{" "}
                    {alertInfo.latestVersion}. Ready for multi-audience release
                    synthesis.
                  </p>
                </div>
              </div>
                      ...
                      <CardHeader className="pb-3 border-b border-border/60 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <Badge
                              variant={
                                rel.status === "PUBLISHED" ? "user" : "outline"
                              }
                              className="text-xs"
                            >
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <div className="relative">
                            ...
                            <CaretDownIcon
                              className="h-3 w-3 absolute right-2 top-2.5 pointer-events-none text-muted-foreground"
                              weight="bold"
                            />
                          </div>
                          ...
                          {selectedProject && (
                            <Link
                              href={`/p/${selectedProject.slug}/${encodeURIComponent(rel.version)}/edit`}
                            >
                              <Button
                                variant="outline"
                                size="sm"
                        ...
                        {currentTab === "dev" && (
                          <div className="space-y-2 text-xs">
                            ...
                            {technicalUpdates.map((u: any, i: number) => (
                              <div
                                key={i}
                                className="p-2 rounded-lg bg-muted/40 border border-border/60 flex items-center gap-2 text-xs"
                              >
                                <Badge
                                  variant="dev"
            ...
            <Card className="p-8 text-center space-y-4">
              <div className="space-y-1">
                <p className="text-sm font-bold text-foreground">
                  No releases published for this project yet.
                </p>
                <p className="text-xs text-muted-foreground max-w-sm mx-auto">
                  Fetch recent commits automatically with 1-click GitHub sync to
                  synthesize your first multi-audience release note.
                </p>
              </div>
              <div className="flex items-center justify-center pt-1">
                ...
                >
                  <SparkleIcon className="h-3.5 w-3.5" weight="fill" /> Generate
                  First Release
                </Button>
              </div>
            </Card>
          ...
          <Card className="shadow-sm border-indigo-500/20">
            <CardHeader className="pb-3 border-b border-border/60">
              <CardTitle className="text-xs font-bold uppercase tracking-wider text-primary flex items-center gap-2">
                <GithubLogoIcon className="h-4 w-4" weight="bold" /> Connected
                Repository
              </CardTitle>
            </CardHeader>
            <CardContent className="p-4 space-y-4">
              <div>
                ...
                <p className="text-xs font-mono text-muted-foreground mt-0.5">
                  {selectedProject?.githubRepo || "github.com/repo"}
                </p>
              </div>
              ...
              <div className="space-y-2 text-xs text-muted-foreground border-t border-border/60 pt-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold text-foreground font-mono">
                    {selectedProject?.releaseCount || 0}
                  </span>
                </div>
