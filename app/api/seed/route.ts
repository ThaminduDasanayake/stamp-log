...
export async function POST() {
  try {
    ...
    const stamplogProject = await db.project.upsert({
    ...
    const reactProject = await db.project.upsert({
    ...
    // 3. Clear existing release notes for these sample projects
    ...
    return Response.json({
      success: true,
      message: "Sample projects and release notes successfully seeded into PostgreSQL!",
    });
  } catch (error: any) {
